...
#define ASSERT_STRINGS_ALL_MATCH(strings, pattern)             \
  do {                                                         \
...
#define ASSERT_STRINGS_ANY_MATCH(strings, pattern)                         \
  do {                                                                     \
    const auto& _strings = (strings);                                      \
    const auto& _pattern = (pattern);                                      \
    bool matched = false;                                                  \
    for (const auto& str : _strings) {                                     \
      if (testing::internal::RE::PartialMatch(                             \
              str, testing::internal::RE(_pattern))) {                     \
        matched = true;                                                    \
        break;                                                             \
      }                                                                    \
    }                                                                      \
    ASSERT_TRUE(matched) << "not one string matched pattern " << _pattern; \
  } while (0)

#define ASSERT_FILE_EXISTS(env, path)                                     \
  do {                                                                    \
    const std::string& _s = path;                                         \
    ASSERT_TRUE(env->FileExists(_s)) << "Expected file to exist: " << _s; \
  } while (0);

#define ASSERT_FILE_NOT_EXISTS(env, path)                                      \
  do {                                                                         \
    const std::string& _s = path;                                              \
    ASSERT_FALSE(env->FileExists(_s)) << "Expected file not to exist: " << _s; \
  } while (0);
